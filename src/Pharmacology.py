    ...
    def main(self):
        ...
        def GetUniprotIDs(self, col):
            ...
            for gene in genes:
                ...
                try:
                    response_df = pd.read_table(response_io, sep='\t', dtype='string')
        ...
        def GetDGIdb(self, method, output_path, date):
            # Genes to analyze
            input_genes = self.df[method].dropna()
            ...
            matchedTerms_DGIdb.to_csv(output_path + self.consDfName + "/CriteriaForSuccess/Drug Interactions/" +
            ...
            if ambiguousTerms_DGIdb.shape[0] == 0 and unmatchedTerms_DGIdb.shape[0] == 0:
                ...
                ambiguousTerms_DGIdb.to_csv(output_path + self.consDfName + "/CriteriaForSuccess/Drug Interactions/" +
                                            self.expName + "_DGIdb_ambiguousTerms_" + str(date) + ".csv", index=False)
                unmatchedTerms_DGIdb.to_csv(output_path + self.consDfName + "/CriteriaForSuccess/Drug Interactions/" +
        ...
        def GetBindingDB(self, method, output_path, date):
            ...
            accessionDict = GetUniprotIDs(input_genes)
            ...
            allTerms_BindingDB.to_csv(output_path + self.consDfName + "/CriteriaForSuccess/Drug Interactions/" +
        ...
        for method in self.methodList:
            ...
            pool.apply_async(GetDGIdb, args = (self, method, self.oPath + method + "/", date))
            pool.apply_async(GetBindingDB, args = (self, method, self.oPath + method + "/", date))
        pool.close()
        pool.join()
