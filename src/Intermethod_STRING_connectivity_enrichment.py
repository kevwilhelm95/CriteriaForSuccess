...
def main(self):
    ...
    evidence_lst = getEvidenceTypes(self.evidences)
    stringNet = selectEvidences(evidence_lst, stringNet)
    stringNet = getCombinedScore(stringNet)

    #Filtering network for edge weight
    edgeWeight = getEdgeWeight(self.confidence)
    ...
    for gL in self.df.columns:
        ...
        for gS in self.GS.columns:
            ...
            uniqueGenes = getUniqueGenes(geneSources)
            uniqueGeneNetwork = getUniqueGeneNetwork(list(uniqueGenes.keys()), stringNet)
            trueConnections = getUniqueGeneNetwork_BwMethodConnections(uniqueGeneNetwork, uniqueGenes)
            ...
            outputUniqueGeneNetwork(uniqueGeneNetwork, uniqueGenes, newOutPutPath)
            geneSetOverlap(geneSets, newOutPutPath)

            #random gene sets b/w set unique gene connectivity w/ degree matching
            uniqueGeneSets = getUniqueGeneCounts(uniqueGenes) #dictionary with unique genes (values) per each set (keys) in true gene lists

            randomSets_Connections = []
            for i in range(250):
                randomSets = createRandomDegreeMatchedSet(uniqueGeneSets, stringNet_allGenes, stringNet_degree_df)
                geneSources = getGeneSources(randomSets, newOutPutPath, 'randomSets')
                uniqueGenes = getUniqueGenes(geneSources)
                uniqueGeneNetwork = getUniqueGeneNetwork(list(uniqueGenes.keys()), stringNet)
                randomConnections = getUniqueGeneNetwork_BwMethodConnections(uniqueGeneNetwork, uniqueGenes)
                randomSets_Connections.append(len(randomConnections))

            mu, sigma = norm.fit(randomSets_Connections)
            z_fit = (len(trueConnections) - mu)/sigma

            plotResultsNorm(randomSets_Connections, trueConnections, newOutPutPath)
            outputRandomSets(randomSets_Connections)